...
torch::Tensor get_scheduler_metadata(
    ...
    const int64_t window_size, const int64_t isa_hint,
    const bool enable_kv_split) {
  cpu_attention::ISA isa = static_cast<cpu_attention::ISA>(isa_hint);
...
void cpu_attn_reshape_and_cache(
    ...
    const torch::Tensor& slot_mapping, const int64_t isa) {
  TORCH_CHECK_EQ(key.dim(), 3);
  TORCH_CHECK_EQ(value.dim(), 3);
  TORCH_CHECK_EQ(kv_cache.dim(), 5);
  ...
  const int64_t num_blocks = kv_cache.size(1);
  const int64_t num_blocks_stride = kv_cache.stride(1);
  const int64_t cache_head_num_stride = kv_cache.stride(2);
  const int64_t block_size = kv_cache.size(3);
  const int64_t block_size_stride = kv_cache.stride(3);
  const int64_t head_dim = key.size(-1);
  const int64_t kv_stride = kv_cache.stride(0);
  // auto kv_vec = kv_cache.unbind(0);

  cpu_attention::ISA isa_tag = static_cast<cpu_attention::ISA>(isa);
        ...
        CPU_ATTN_DISPATCH_CASE_HEADDIM(head_dim, [&] {
          CPU_ATTN_DISPATCH_IMPL(isa_tag, [&]() {
            attn_impl::reshape_and_cache(
                ...
                kv_cache.data_ptr<scalar_t>() + kv_stride,
                // kv_vec[0].data_ptr<scalar_t>(),
                // kv_vec[1].data_ptr<scalar_t>(),
...
void cpu_attention_with_kv_cache(
  ...
  input.num_kv_heads = kv_cache.size(2);
  input.block_size = kv_cache.size(3);
  input.query = query.data_ptr();
  input.query_num_tokens_stride = query.stride(0);
  input.query_num_heads_stride = query.stride(1);
  input.cache_num_blocks_stride = kv_cache.stride(1);
  input.cache_num_kv_heads_stride = kv_cache.stride(2);
  const int64_t kv_stride = kv_cache.stride(0);
  input.blt_num_tokens_stride = block_table.stride(0);
  // auto kv_vec = kv_cache.unbind(0);
  // input.key_cache = kv_vec[0].data_ptr();
  // input.value_cache = kv_vec[1].data_ptr();
      ...
      query.scalar_type(), "cpu_attention_with_kv_cache", [&]() {
        CPU_ATTN_DISPATCH_CASE_HEADDIM(query.size(2), [&] {
          CPU_ATTN_DISPATCH_IMPL(input.metadata->isa, [&]() {
            input.key_cache = kv_cache.data_ptr<scalar_t>();
            input.value_cache = kv_cache.data_ptr<scalar_t>() + kv_stride;
