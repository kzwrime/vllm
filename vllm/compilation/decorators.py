...
def _should_eager_for_xcpu_gdn_prefill_mixed() -> bool:
    ...
    This is a narrow XCPU experiment for vLLM 0.19.x. GDN decode-only has a
    compact fast path, but prefill/mixed still goes through the larger Python
    custom op. Since this compile mode traces one route, bypass compilation
    until the current forward is decode-only for every GDN metadata object.
    """
