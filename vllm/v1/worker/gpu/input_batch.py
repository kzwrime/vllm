...
if not HAS_TRITON:
    prepare_prefill_inputs = resolve_fallback_kernel(
        prepare_prefill_inputs,
        "prepare_prefill_inputs",
    )
    prepare_pos_seq_lens = resolve_fallback_kernel(
        prepare_pos_seq_lens,
        "prepare_pos_seq_lens",
    )
    combine_sampled_and_draft_tokens = resolve_fallback_kernel(
        combine_sampled_and_draft_tokens,
        "combine_sampled_and_draft_tokens",
    )
    get_num_sampled_and_rejected = resolve_fallback_kernel(
        get_num_sampled_and_rejected,
        "get_num_sampled_and_rejected",
    )
    post_update = resolve_fallback_kernel(
        post_update,
        "post_update",
    )
    post_update_pool = resolve_fallback_kernel(
        post_update_pool,
        "post_update_pool",
    )
    expand_idx_mapping = resolve_fallback_kernel(
        expand_idx_mapping,
        "expand_idx_mapping",
    )
