...
class MultiprocRPCExecutor(Executor):
    ...
    def __init__(self, vllm_config: VllmConfig, monitor_workers: bool = False):
    ...
    def _init_executor(self) -> None:
        ...
        if self.parallel_config.node_rank_within_dp == 0:
            ...
            self.rpc_broadcast_mq = MessageQueue(
                self.world_size,
                0,  # TODO: dynamically detect the number of local readers
        ...
        if not getattr(self, "shutting_down", False):
            ...
            if self.rpc_broadcast_mq:
                ...
                try:
                    ...
                    logger.warning("Could not send shutdown command to workers: %s", e)
...
class WorkerProc:
    ...
    def _init_message_queues(
        ...
        if vllm_config.parallel_config.nnodes_within_dp == 1:
            ...
            self.worker_response_mq = MessageQueue(1, 0)
    ...
    def make_worker_process(
        ...
        logger.info(
            "Executor waiting for worker with rank %d / %d to connect to %s:%d",
            rank,
            world_size,
            display_ip,
            port,
        ...
        logger.info("Accepted connection from worker rank %d at %s", rank, str(addr))
        ...
        config_payload = pickle.dumps(process_kwargs)
        ...
        return UnreadyWorkerProcHandle(None, rank, conn)
    ...
    def wait_for_ready(
        ...
        e = Exception("WorkerProc initialization failed. See logs for details.")

        ready_proc_handles: list[WorkerProcHandle | None] = [None] * len(
            unready_proc_handles
        )

        for unready_proc_handle in unready_proc_handles:
            rank = unready_proc_handle.rank
            conn = unready_proc_handle.ready_pipe
            assert conn is not None
            try:
                with conn:
                    ...
                    if not len_data:
                        raise ConnectionAbortedError(
                            "Worker %d disconnected before sending ready signal.", rank
                        )
                    ...
                    logger.info("Received ready signal from worker rank %d.", rank)
                    worker_response_mq = MessageQueue.create_from_handle(
                        response["handle"], 0
                    )
                    ready_proc_handles[rank] = WorkerProcHandle.from_unready_handle(
        ...
        try:
            ...
            while not connected and (time.time() - start_time) < max_retry_time:
                try:
                    ...
                    if (time.time() - start_time) < max_retry_time:
                        time.sleep(retry_interval)  # Sleep briefly before retry
                    else:
            ...
            with ready_socket:
                ...
                if rank == 0:
                    ...
                    ready_socket.sendall(distributed_init_method.encode("utf-8"))
            ...
            logger.exception("WorkerProc failed during initialization or execution.")
