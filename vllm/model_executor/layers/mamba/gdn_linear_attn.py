...
class GatedDeltaNetAttention(PluggableLayer, MambaBase):
    ...
    def __init__(
        ...
        self.use_in_proj_qkvzba = self._should_use_in_proj_qkvzba(
            config=config,
        ...
        if not self.use_in_proj_qkvzba:
    ...
    def _should_use_in_proj_qkvzba(
        self,
        config: Qwen3NextConfig,
        vllm_config: VllmConfig,
        create_in_proj_qkvz: bool,
    ) -> bool:
        if not envs.VLLM_XCPU_FUSE_GDN_IN_PROJ_QKVZBA:
            return False
        if not create_in_proj_qkvz:
            logger.warning_once(
                "VLLM_XCPU_FUSE_GDN_IN_PROJ_QKVZBA is disabled for %s "
                "because LoRA uses separate GDN input projections.",
                self.prefix,
            )
            return False
        if self.gqa_interleaved_layout:
            logger.warning_once(
                "VLLM_XCPU_FUSE_GDN_IN_PROJ_QKVZBA is disabled for %s "
                "because interleaved GDN layout is not supported.",
                self.prefix,
            )
            return False
        if vllm_config.model_config.dtype != torch.bfloat16:
            logger.warning_once(
                "VLLM_XCPU_FUSE_GDN_IN_PROJ_QKVZBA is disabled for %s "
                "because only bf16 is supported.",
                self.prefix,
            )
            return False
        if getattr(config, "model_type", None) not in (
            ...
            logger.warning_once(
                "VLLM_XCPU_FUSE_GDN_IN_PROJ_QKVZBA is disabled for %s "
                "because only Qwen3.5/Qwen3.6 GDN configs are supported.",
                self.prefix,
            )
            return False
        logger.info_once(
            "Using fused GDN in_proj_qkvzba for Qwen3.5/Qwen3.6 bf16 no-LoRA path.",
            scope="local",
        )
        return True
    ...
    def forward(
        ...
        if self.use_in_proj_qkvzba:
            ...
            mixed_qkv, z, ba = mixed_qkvzba.split([qkv_size, z_size, ba_size], dim=-1)
            z = z.reshape(z.size(0), -1, self.head_v_dim)
    ...
    def _forward_core_decode_non_spec_compile(
        ...
        num_actual_tokens_cpu = attn_metadata_i.num_actual_tokens_cpu
        if non_spec_state_indices_tensor is None:
            raise RuntimeError(
                "XCPU GDN decode-only compile path requires "
                "non_spec_state_indices_tensor."
            )
        if num_actual_tokens_cpu is None:
            raise RuntimeError(
                "XCPU GDN decode-only compile path requires num_actual_tokens_cpu."
            )
        if not hasattr(self, "kv_cache"):
            raise RuntimeError(
                "XCPU GDN decode-only compile path requires bound GDN KV cache."
            )
        state_handle = getattr(self, "_xcpu_gdn_decode_state_handle_019", None)
        if state_handle is None:
            raise RuntimeError(
                "XCPU GDN decode-only compile path requires a registered "
                "GDN decode state handle."
            )
